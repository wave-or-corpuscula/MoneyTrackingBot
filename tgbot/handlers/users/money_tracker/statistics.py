...
from tgbot.utils import Database, Multitool, ScreenManager, ExcelManager
...
async def get_report(callback: types.CallbackQuery, state: FSMContext, bot: Bot, db: Database):
    ...
    excel = ExcelManager()
    filename = excel.create_excel_report(callback.from_user.id, callback.from_user.full_name, db)
